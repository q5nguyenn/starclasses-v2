  ...
  <div class="search-result">
    <div class="profile-frame" data-frame="2">
      <div class="profile-frame-top my-3">
        <div class="profile-frame-title">UPDATE USER</div>
      </div>
        ...
        <form class="profile-frame show" data-frame="0" novalidate enctype="multipart/form-data"
          ...
          <div class="profile-frame-wrapper profile-frame-flex profile-frame-flex-mobile profile-frame-flex-gap">
            <div class="profile-frame-column">
                ...
                <select class="form-control js-example-tokenizer" multiple="multiple" name="roles[]" id="roles">
                  ...
                  @foreach ($roles as $role)
                    <option value="{{ $role->id }}"
                      {{ in_array($role->id, old('roles', $user_roles)) ? 'selected' : '' }}>
                      {{ $role->name }}</option>
                  @endforeach
                </select>
                <div class="input-item-error">&nbsp;
              ...
              <div class="input-item">
                <textarea name="description" id="description" class="input" style="height:150px">{{ old('description', $user->description) }}</textarea>
            ...
            <div class="profile-frame-column">
              ...
              <label for="password">Password change</label> <input type="checkbox" class="form-check-input"
                name="change-password" id="change-password">
              <div class="input-item ">
                <input type="password" class="input" id="password" name="password" value="123456" disabled />
                <span class="focus-input"></span>
              </div>
              <div class="input-item-error">&nbsp;
                @error('password')
              ...
              <button class="button-fa flex-right btn-update-user" type="submit" id="btn-update">Update</button>
            </div>
          </div>
        </form>
      </div>
...
@section('js')
  ...
  <script>
    $('#change-password').change(function(e) {
      e.preventDefault();
      if ($(this).is(":checked")) {
        $('#password').val('');
