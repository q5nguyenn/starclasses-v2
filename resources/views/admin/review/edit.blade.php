  ...
  <div class="search-result">
    <div class="profile-frame" data-frame="2">
      <div class="profile-frame-top my-3">
        <div class="profile-frame-title">UPDATE REVIEW</div>
      </div>
      <div class="profile-frame-wrapper">
        <form class="profile-frame-flex profile-frame-width" id="active-voucher" novalidate method="post"
          ...
          <div class="input-item">
            <textarea name="content" id="content" class="input" style="height:100px">{{ old('content', $review->content) }}</textarea>
          ...
          <select class="form-select" aria-label="Default select example" name="rate" id="rate">
            ...
            @for ($i = 1; $i <= 5; $i++)
              <option value="{{ $i }}" {{ old('rate', $review->rate) == $i ? 'selected' : '' }}>
                {{ $i }} star
              </option>
            @endfor
          ...
          <select class="form-select" aria-label="Default select example" name="status" id="status">
            ...
            <option value="1" {{ old('status', $review->status) == 1 ? 'selected' : '' }}>Ok</option>
            <option value="0" {{ old('status', $review->status) == 0 ? 'selected' : '' }}>Report</option>
            <option value="2" {{ old('status', $review->status) == 2 ? 'selected' : '' }}>Hide</option>
          </select>
          <div class="input-item-error">&nbsp;
            @error('status')
              {{ $message }}
