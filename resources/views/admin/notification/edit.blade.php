  ...
  <div class="search-result">
    <div class="profile-frame" data-frame="2">
      <div class="profile-frame-top my-3">
        <div class="profile-frame-title">UPDATE NOTIFICATION</div>
      </div>
      <div class="profile-frame-wrapper">
        <form class="profile-frame-flex profile-frame-width" id="active-voucher" novalidate method="post"
          action="{{ route('admin.notification.update', ['id' => $notification->id]) }}">
          @csrf
          ...
          <select class="form-select js-user-placeholder" multiple="multiple" aria-label="Default select example"
            ...
            @foreach ($users as $user)
              <option value="{{ $user->id }}"
                {{ in_array($user->id, old('users', $notification->users->pluck('id')->toArray())) ? 'selected' : '' }}>
                {{ $user->email }}</option>
            @endforeach
          </select>
          ...
          <div class="input-item">
            ...
            <textarea id="description" name="description" class="form-control" rows="3" placeholder="description" required>{{ old('description', $notification->content) }}</textarea>
