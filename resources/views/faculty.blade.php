@extends('master')
@section('title')
  <title>Star Classes - Faculty</title>
@endsection
@section('css')
    ...
    <div class="search-top-tag">
      <div class="search-top-tag-wrapper">
        <div class="course-tree">
          ...
          <a id="section" href="{{ route('faculty', ['slug' => $faculty->slug]) }}" id="section">
            {{ $faculty->name }}
          </a>
          {{-- <i class="bi bi-chevron-double-right"></i>
                    <a id="subject" href="#" id="subject"></a> --}}
        </div>
      </div>
    </div>
      ...
      <div class="search-filter">
        <div class="search-filter-title">
          <i class="bi bi-sliders"></i> <span>Filter</span><button class="button-fa" id="clear-filter"
            data-clear="{{ route('faculty', ['slug' => $faculty->slug]) }}">Clear Filter</button>
        </div>
        <form action="{{ route('faculty.sort', ['slug' => $faculty->slug]) }}" id="filter" method="get">
      ...
      <div class="search-result">
        <div class="search-result-top">
          ...
          <div class="search-count">
            ...
            {{ count($faculty->courses()->get()) < 10 ? '0' . count($faculty->courses()->get()) : count($faculty->courses()->get()) }}
            courses
...
@section('js')
  <script>
    ...
    let count = {{ $count }};
    let faculty_slug = '{{ $faculty->slug }}';
  </script>
  <script type="module" src="{{ asset('js/faculty.js') }}"></script>
@endsection
